...
void FWakatimeIntegrationModule::StartupModule()
{
	...
	FCoreUObjectDelegates::OnObjectPreSave.AddRaw(this, &FWakatimeIntegrationModule::OnObjectPreSave);
...
void FWakatimeIntegrationModule::OnObjectPreSave(UObject* SavedObject, FObjectPreSaveContext SaveContext)
{
	int64 now = GetCurrentTime();
	if ((now - LastAssetPushTime) < SaveDebounce) {
		return;
